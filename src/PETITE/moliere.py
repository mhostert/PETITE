    ...
    if x <= 100.0:
        if x > 1e-6:
            return 2.0 * np.exp(-x) * (x - 1.0) * (
                special.expi(x) - math.log(x)
            ) - 2.0 * (1.0 - 2.0 * np.exp(-x))
        else:
        ...
        return 2.0 / np.power(x, 2) + 8.0 / np.power(x, 3) + 36.0 / np.power(x, 4)
...
def moliere_cdf(x, B):
    ...
    ## EDGE CASE 2
        ...
        return integrate.quad(integrand, 0.0, x)[0]
...
def get_b(t, beta, A, Z, z):
    ...
    expb = (
        ...
        * np.power(Z, 1.0 / 3.0)
        * np.power(z, 2.0)
        / (np.power(beta, 2) * A * (1.0 + 3.34 * alpha**2))
    )
...
def get_chic_squared(t, beta, A, Z, z):
    ...
    p = me_in_inv_cm * beta / np.sqrt(1.0 - beta**2)
...
def get_chic_squared_alt(t, beta, A, Z, z):
    ...
    p = (
        me_in_MeV * beta / np.sqrt(1.0 - beta**2)
    )  # momentum has to be in MeV, see below Eq. 2 in Lynch & Dahl, 1991
    return 0.157 * Z * (Z + 1) * (t / A) * np.power(z / (p * beta), 2.0)
...
def get_chia_squared_alt(beta, A, Z, z):
    ...
    return (
        2.007e-5
        * np.power(Z, 2.0 / 3.0)
        * (1.0 + 3.34 * np.power(Z * z * alpha_em / beta, 2.0))
        / p**2
    )
...
def generate_moliere_angle_simplified(t_over_X0, beta, z):
    ...
    """
    p = m_electron * beta / np.sqrt(1.0 - beta**2)
    theta0 = (
        (13.6 * MeV)
        * np.fabs(z)
        * np.sqrt(t_over_X0)
        * (1.0 + 0.038 * math.log(t_over_X0))
        / (beta * p)
    )
    ...
    return random.choice([-1, 1]) * np.sqrt(2.0 * random.expovariate(1.0 / (theta0**2)))
...
def generate_moliere_angle_simplified_alt(t, beta, A, Z, z):
    ...
    omega = chic2 / chia2
    v = 0.5 * omega / (1.0 - F)
    theta0 = np.sqrt(chic2 * ((1.0 + v) * math.log(1.0 + v) / v - 1) / (1.0 + F**2))
    # print("Lynch and Dah theta0 = ", theta0)
    # theta0 is the standard deviation for the plane angle, but we want to generate the space angle
    # in the small angle approximation the space angle is theta = sqrt(thetax^2 + thetay^2)
    return random.choice([-1, 1]) * np.sqrt(
        random.gauss(0.0, theta0) ** 2 + random.gauss(0.0, theta0) ** 2
    )
...
def get_rotation_matrix(v):
    ...
    if np.fabs(vx) > 0.0 and np.fabs(vy) > 0.0:
        ta = np.fabs(vy / vx)
        a = np.arctan(ta)
    ...
    Ra = np.array([[ca, -sa, 0.0], [sa, ca, 0.0], [0.0, 0.0, 1.0]])

    # Find second rotation to set x component to 0
    vxp = vx * ca - vy * sa
    if np.fabs(vz) > 0.0 and np.fabs(vxp) > 0.0:
        tb = np.fabs(vxp / vz)
        b = np.arctan(tb)

        if vz > 0.0 and vxp > 0.0:
            b = -b
        if vz < 0.0 and vxp > 0.0:
            b = -(np.pi - b)
        if vz < 0.0 and vxp < 0.0:
    ...
    Rb = np.array([[cb, 0.0, sb], [0.0, 1.0, 0.0], [-sb, 0.0, cb]])
...
def get_scattered_momentum_fast(p4, t, A, Z, rescale_MCS=1):
    ...
    Rtheta = np.array([[1.0, 0.0, 0.0], [0.0, cth, -sth], [0.0, sth, cth]])
    Rphi = np.array([[cph, -sph, 0.0], [sph, cph, 0.0], [0.0, 0.0, 1.0]])

    p3_new = p3_norm * np.matmul(Rphi, np.matmul(Rtheta, np.array([0.0, 0.0, 1.0])))
