...
class Shower:
    ...
    def set_cross_sections(self):
        ...
        self._minimum_calculable_energy = {
            11: np.min(
                [self._brem_cross_section[0][0], self._moller_cross_section[0][0]]
            ),
            -11: np.min(
                [
                    self._brem_cross_section[0][0],
                    self._bhabha_cross_section[0][0],
                    self._annihilation_cross_section[0][0],
                ]
            ),
            22: np.min(
                [
                    self._pair_production_cross_section[0][0],
                    self._compton_cross_section[0][0],
                ]
            ),
        }
    ...
    def set_NSigmas(self):
        ...
        bhabha_moller_energies = np.geomspace(
            3 * m_electron + self._Ee_min, _Ee_MAX, 101
        )
        ...
        II_y_Brem = np.array(
            [
                quad(self._NSigmaBrem, BS[0][0], BS[i][0], full_output=1)[0]
        ...
        II_y_PP = np.array(
            [
                quad(self._NSigmaPP, PPS[0][0], PPS[i][0], full_output=1)[0]
        ...
        II_y_Ann = np.array(
            [
                quad(self._NSigmaAnn, AnnS[0][0], AnnS[i][0], full_output=1)[0]
        ...
        II_y_Comp = np.array(
            [
                quad(self._NSigmaComp, CS[0][0], CS[i][0], full_output=1)[0]
        ...
        II_y_Moller = np.array(
            [
                quad(
                    self._NSigmaMoller,
                    bhabha_moller_energies[0],
                    bhabha_moller_energies[i],
                    full_output=1,
                )[0]
                for i in range(len(bhabha_moller_energies))
            ]
        )
        self._interaction_integral_Moller = interp1d(
            bhabha_moller_energies, II_y_Moller, fill_value=0.0, bounds_error=False
        )
    ...
    def _positron_exponential_factor(self, E, Ei):
        ...
        n_sigma_diff = (
            ...
            + (
                self._interaction_integral_Bhabha(Ei)
                - self._interaction_integral_Bhabha(E)
            )
        )
    ...
    def get_mfp(self, particle):
        ...
        if type(particle) is not Particle and (
            type(particle) is list or type(particle) is np.ndarray
        ):
            PID, Energy = particle
        else:
        ...
        if PID == 22:
            ...
            return (
                cmtom
                * (
                    self._NSigmaBrem(Energy)
                    + self._NSigmaAnn(Energy)
                    + self._NSigmaBhabha(Energy)
                )
                ** -1
            )
    ...
    def draw_sample(self, Einc, LU_Key=-1, process="PairProd", VB=False):
        ...
        if LU_Key < 0 or LU_Key > len(sample_list[process]):
            ...
            if LU_Key < 0:
                LU_Key = 0
                print(
                    "Warning: sampling below minimum energy for process" + str(process)
                )
            if LU_Key >= len(sample_list[process]):
                LU_Key = len(sample_list[process]) - 1
                print(
                    "Warning: sampling above maximum energy for process" + str(process)
                )
        ...
        integrand = vg.Integrator(
            map=adaptive_map,
            max_nhcube=1,
            nstrat=np.ones(dimensionalities[process]),
            neval=neval_vegas,
        )
    ...
    def sample_scattering(self, p0, process, VB=False):
        ...
        if E0 <= np.max(
            [
                self._minimum_calculable_energy[p0.get_ids()["PID"]],
                self.min_energy,
                p0.get_ids()["mass"],
            ]
        ):
    ...
    def propagate_particle(self, Part0, Losses=False, MS=False):
        ...
        if Part0.get_ended() is True:
            ...
            if (
                (np.linalg.norm(Part0.get_p0() - Part0.get_pf()) > 0.0)
                or (np.linalg.norm(Part0.get_r0() - Part0.get_rf()) > 0.0)
                or (Part0.get_ended())
            ):
                raise ValueError(
                    "propagate_particle() should only be called \
                for a particle with pf = p0 and rf = r0 and get_ended() == False"
                )

            particle_min_energy = np.max(
                [
                    self._minimum_calculable_energy[Part0.get_ids()["PID"]],
                    self.min_energy,
                    Part0.get_ids()["mass"],
                ]
            )
            ...
            if not Losses:
                ...
                Part0.set_rf(
                    [x0 + PHat[0] * dist, y0 + PHat[1] * dist, z0 + PHat[2] * dist]
                )
                ...
                while not hard_scatter and Part0.get_pf()[0] >= particle_min_energy:
                    ...
                    # If no hard scatter propagate particle and account for energy loss
                        ...
                        if pf0 > 0.0:
                            ...
                            Part0.set_rf(
                                [
                                    x_current + pfx / pf0 * delta_z,
                                    y_current + pfy / pf0 * delta_z,
                                    z_current + pfz / pf0 * delta_z,
                                ]
                            )
                        if MS:
                            Part0.set_pf(
                ...
                if Part0._pf[0] < particle_min_energy:
                    ...
                    mfp = self.get_mfp(Part0)
                    last_increment = mfp * np.log(
                        1.0 / (1.0 + (np.exp(-delta_z / mfp) - 1) * distC)
                    )
                ...
                if pf0 > 0.0:
                    ...
                    Part0.set_rf(
                        [
                            x_current + pfx / pf0 * last_increment,
                            y_current + pfy / pf0 * last_increment,
                            z_current + pfz / pf0 * last_increment,
                        ]
                    )
                if MS:
                    Part0.set_pf(
                ...
                if ap.get_ended() is True:
                    ...
                    if ap.get_ids()["stability"] == "short-lived":
                        ...
                        if ap.get_ids()["PID"] == 11:
                            choices0 = self._NSigmaBrem(
                                ap.get_pf()[0]
                            ), self._NSigmaMoller(ap.get_pf()[0])
                            SC = np.sum(choices0)
                            if SC == 0.0 or np.isnan(SC):
                                continue
                            choices0 = choices0 / SC
                            draw = np.random.choice(["Brem", "Moller"], p=choices0)
                            newparticles = self.sample_scattering(
                                ap, process=draw, VB=VB
                            )
                        elif ap.get_ids()["PID"] == -11:
                            choices0 = (
                                self._NSigmaBrem(ap.get_pf()[0]),
                                self._NSigmaAnn(ap.get_pf()[0]),
                                self._NSigmaBhabha(ap.get_pf()[0]),
                            )
                            SC = np.sum(choices0)
                            if SC == 0.0 or np.isnan(SC):
                                continue
                            ...
                            choices0 = self._NSigmaPP(ap.get_pf()[0]), self._NSigmaComp(
                                ap.get_pf()[0]
                            )
                            SC = np.sum(choices0)
                            if SC == 0.0 or np.isnan(SC):
...
def event_display(all_particles):
    ...
    ymax = np.max(
        [
            np.max([np.abs(p.get_rf()[0]) for p in all_particles]),
            np.max([np.abs(p.get_rf()[1]) for p in all_particles]),
        ]
    )
    ymin = -ymax
    ...
    for ki0 in all_particles:
        ...
        if ki0.get_pid() == 22:
            with matplotlib.rc_context({"path.sketch": (5, 15, 1)}):
                ax.plot(
                    [ki[2], ki[5]], [ki[0], ki[3]], lw=1, ls="-", color="g", alpha=0.5
                )
        if ki0.get_pid() == 11:
    ...
    for ki0 in all_particles:
        ...
        if ki0.get_pid() == 22:
            with matplotlib.rc_context({"path.sketch": (5, 15, 1)}):
                ax.plot(
                    [ki[2], ki[5]], [ki[1], ki[4]], lw=1, ls="-", color="g", alpha=0.5
                )
        if ki0.get_pid() == 11:
...
def transverse_position(particle, z):
    ...
    T = (
        z - z0
    ) / pz  # proxy for the time of propagation from z0 to z in the longitudinal direction
...
def detector_cut(
    ...
    if energy_cut is not None:
        ...
        particle_list = particle_list[
            np.where(energies < energy_cut[1], True, False)
            * np.where(energies > energy_cut[0], True, False)
        ]
    ...
    rT = np.linalg.norm(
        [transverse_position(p0, detector_positions) for p0 in particle_list], axis=1
    )

    pass_cuts_where = np.transpose(
        np.where(rT > detector_inner_radius, True, False)
        * np.where(rT < detector_radius, True, False)
    )
    ...
    if method == "Sample":
        ...
        return [
            np.sum([p0.get_ids()["weight"] for p0 in pass_cuts[ii]])
            / np.sum([p0.get_ids()["weight"] for p0 in particle_list])
            for ii in range(len(pass_cuts))
        ]
