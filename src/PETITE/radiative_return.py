...
def fl_kf_scaled(x, s):
    ...
    beta = (2.0 * alpha_em / np.pi) * (np.log(s / m_electron**2) - 1.0)
    return (beta / 16.0) * (
        (8.0 + 3.0 * beta) - 4.0 * (1.0 + x) * np.power(1.0 - x, 1.0 - beta / 2.0)
    )
...
def lepton_luminosity_integrand(s, x, y):
    ...
    if (x > 0.0 and x <= 1.0) and (y > 0.0 and y <= 1.0) and x <= y:
...
def transformed_lepton_luminosity_integrand(s, y, u):
    ...
    beta = (2.0 * alpha_em / np.pi) * (np.log(s / m_electron**2) - 1.0)
    x = 1.0 - np.power(u, 2.0 / beta)

    # The factor 2/beta comes from the variable transformation
    # print(fl_kf(y/x,s),"\t", fl_kf_scaled(x,s),"\t",(1./x) )
    return fl_kf(y / x, s) * fl_kf_scaled(x, s) * (1.0 / x) * (2.0 / beta)


_lumi_int_fill_val = np.nan
log_lumi_integral_list = np.log10(lumi_integral_list + 1e-99)
log_lumi_integral_interp = LinearNDInterpolator(
    list(zip(log_lumi_integral_list[:, 0], log_lumi_integral_list[:, 1])),
    log_lumi_integral_list[:, 2],
    fill_value=_lumi_int_fill_val,
)
...
def radiative_return_cross_section(s, mA):
    eps = 1.0
    betaf = np.sqrt(1.0 - 4.0 * (m_electron**2) / (mA**2))
    ...
    return prefac * lumi_factor * 3.89379e08  # 1/GeV^2 -> pb
