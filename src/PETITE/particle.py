import numpy as np
import PETITE.physical_constants as pconst

mass_dict = {
    11: pconst.m_electron,
    -11: pconst.m_electron,
    22: 0.0,
    13: pconst.m_muon,
    -13: pconst.m_muon,
    ...
    -211: pconst.m_pi_pm,
    221: pconst.m_eta,
    331: pconst.m_eta_prime,
    2212: pconst.m_proton,
    223: pconst.m_omega,
    12: 0.0,
    -12: 0.0,
    14: 0.0,
...
meson_twobody_branchingratios = {
    pid0: meson_decay_dict[pid0][0][0] for pid0 in meson_decay_dict.keys()
}
...
class Particle:
    ...
    def rotation_matrix(self):
        ...
        return [
            [np.cos(ThZ) * np.cos(PhiZ), -np.sin(PhiZ), np.sin(ThZ) * np.cos(PhiZ)],
    ...
    def boost_matrix(self):
        ...
        return [
            ...
            [
                gamma * betax,
                1 + (gamma - 1) * betax**2 / beta**2,
                (gamma - 1) * betax * betay / beta**2,
                (gamma - 1) * betax * betaz / beta**2,
            ],
            [
                gamma * betay,
                (gamma - 1) * betay * betax / beta**2,
                1 + (gamma - 1) * betay**2 / beta**2,
                (gamma - 1) * betay * betaz / beta**2,
            ],
            [
                gamma * betaz,
                (gamma - 1) * betaz * betax / beta**2,
                (gamma - 1) * betaz * betay / beta**2,
                1 + (gamma - 1) * betaz**2 / beta**2,
            ],
        ]
    ...
    def two_body_decay(self, p1_dict, p2_dict, angular_information="Isotropic"):
        ...
        if ("mass") not in p1_dict.keys():
            if ("PID") not in p1_dict.keys():
                raise ValueError(
                    "Masses must be included in `p1_dict' when calling two_body_decay()"
                )
            else:
                try:
        ...
        if ("mass") not in p2_dict.keys():
            if ("PID") not in p2_dict.keys():
                raise ValueError(
                    "Masses must be included in `p2_dict' when calling two_body_decay()"
                )
            else:
                try:
        ...
        p1_four_vector_RF = [
            E1,
            -pF * sin_theta * np.sin(phi),
            -pF * sin_theta * np.cos(phi),
            -pF * cos_theta,
        ]
        p2_four_vector_RF = [
            E2,
    ...
    def dalitz_range(self, m12sq, m1, m2, m3, M):
        ...
        m23sqmin = (E2s + E3s) ** 2 - (
            np.sqrt(E2s**2 - m2**2) + np.sqrt(E3s**2 - m3**2)
        ) ** 2
        m23sqmax = (E2s + E3s) ** 2 - (
            np.sqrt(E2s**2 - m2**2) - np.sqrt(E3s**2 - m3**2)
        ) ** 2
    ...
    def three_body_decay(
        ...
        p2_dict,
        p3_dict,
        dalitz_information="Flat",
        angular_information="Isotropic",
    ):
        mX = self._mass
        if ("mass") not in p1_dict.keys():
            if ("PID") not in p1_dict.keys():
                raise ValueError(
                    "Masses must be included in `p1_dict' when calling three_body_decay()"
                )
            else:
                try:
                    p1_dict["mass"] = mass_dict[p1_dict["PID"]]
        ...
        if ("mass") not in p2_dict.keys():
            if ("PID") not in p2_dict.keys():
                raise ValueError(
                    "Masses must be included in `p2_dict' when calling three_body_decay()"
                )
            else:
                try:
                    p2_dict["mass"] = mass_dict[p2_dict["PID"]]
        ...
        if ("mass") not in p3_dict.keys():
            if ("PID") not in p3_dict.keys():
                raise ValueError(
                    "Masses must be included in `p3_dict' when calling three_body_decay()"
                )
            else:
                try:
        ...
        while sample_found is False:
            m12sq, m23sq = np.random.uniform(m12sq_min, m12sq_max), np.random.uniform(
                m23sq_min, m23sq_max
            )
        ...
        p1_four_vector_RF = [
            E1,
            -p1 * sin_theta * np.sin(phi),
            -p1 * sin_theta * np.cos(phi),
            -p1 * cos_theta,
        ]
        p2_four_vector_RF = [
            E2,
            p2
            * (
                sin_theta_12 * np.cos(phi) * np.sin(gamma)
                - sin_theta_12 * np.cos(gamma) * cos_theta * np.sin(phi)
                - cos_theta_12 * sin_theta * np.sin(phi)
            ),
            p2
            * (
                -np.cos(phi)
                * (sin_theta_12 * np.cos(gamma) * cos_theta + cos_theta_12 * sin_theta)
                - sin_theta_12 * np.sin(gamma) * np.sin(phi)
            ),
            p2 * (sin_theta_12 * np.cos(gamma) * sin_theta - cos_theta_12 * cos_theta),
        ]
        p3_four_vector_RF = [
            E3,
            -p1_four_vector_RF[1] - p2_four_vector_RF[1],
            -p1_four_vector_RF[2] - p2_four_vector_RF[2],
            -p1_four_vector_RF[3] - p2_four_vector_RF[3],
        ]
    ...
    def decay_particle(self):
        if self.get_ids()["PID"] not in meson_decay_dict.keys():
            raise ValueError(
                "Decay options for particle not specified. Edit dictionary in 'particle.py' to include it"
            )
        ...
        if len(decay_options) == 1:
            ...
            branching_ratios = [
                decay_options[ii][0] for ii in range(len(decay_options))
            ]
            br_sum = np.sum(branching_ratios)
            choice_weights = branching_ratios / br_sum
            decay = decay_options[
                np.random.choice(range(len(decay_options)), p=choice_weights)
            ][1]
        ...
        if len(decay) > 2:
            ...
            p1_dict = {
                "PID": decay[0],
                "mass": mass_dict[decay[0]],
                "weight": self.get_ids()["weight"] * br_sum,
                "ID": 2 * (self.get_ids()["ID"]),
                "generation_process": "SMDecay",
                "generation_number": (self.get_ids()["generation_number"] + 1),
                "production_time": self.get_ids()["decay_time"],
            }
            p2_dict = {
                "PID": decay[1],
                "mass": mass_dict[decay[1]],
                "weight": self.get_ids()["weight"] * br_sum,
                "ID": 2 * (self.get_ids()["ID"]) + 1,
                "generation_process": "SMDecay",
                "generation_number": (self.get_ids()["generation_number"] + 1),
                "production_time": self.get_ids()["decay_time"],
            }
