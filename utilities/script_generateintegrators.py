...
from PETITE import generate_integrators
...
def main(
    doSM=True, doDark=True, Ee_min=0.0016, Ee_max=100.0, Npoints=100, path=DEFAULT_PATH
):
    ##################################
    ##         SM processes         ##
    ##################################
    ...
    if doSM:
        for process in processes_to_do:
            if os.path.exists(
                ...
                generate_integrators.stitch_integrators(
                    training_params["save_location"] + "/" + process + "/"
                )
                generate_integrators.cleanup(
                    training_params["save_location"] + "/" + process + "/"
                )
        # List of processes to run find_maxes on, need not be the same as list above
        find_maxes_processes_to_do = [
            "Brem",
            "PairProd",
            "Comp",
            "Ann",
        ...
        generate_integrators.call_find_maxes(
            processing_params, find_maxes_processes_to_do
        )
    ...
    training_params = {
        "verbosity": True,
        "initial_energy_list": initial_energy_list_general,
        ...
        for mV in mV_list:
            for process in processes_to_do:
                ...
                if os.path.exists(
                    ...
                    generate_integrators.organize_directories_final(
                        training_params["save_location"]
                        + process
                        + "/mV_"
                        + str(int(np.floor(mV * 1000.0)))
