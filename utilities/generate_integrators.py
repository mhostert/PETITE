...
def generate_vector_mass_string(mV):
    return str(int(np.floor(mV * 1000.0))) + "MeV"
...
def make_readme(params, process, process_directory):
    ...
    if process == "DarkBrem":
        line = "\nTarget has (Z, A, mass) = ({atomic_Z}, {atomic_A}, {atomic_mass})\n".format(
            atomic_Z=params["Z_T"], atomic_A=params["A_T"], atomic_mass=params["mT"]
        )
        readme_file.write(line)
    readme_file.write("\n\nEnergy/GeV |  Filename\n\n")
    for index, energy in enumerate(params["initial_energy_list"]):
        line = "{en:9.3f}  |  {proc}_{indx}.p\n".format(
            en=energy, indx=index, proc=process
        )
        readme_file.write(line)
...
def run_vegas_in_parallel(
    params, process, verbosity_mode, process_directory, energy_index, overwrite=False
):
...
def make_integrators(params, process, paralellize=True, overwrite=False):
    ...
    # vec_mass_string = generate_vector_mass_string(mV)  # NOTE: This is not used anywhere
...
def main(args):
    ...
    if args.mV == 0 or not (args.process == ["DarkBrem"]):  # doing SM processes
        ...
        for process in process_list_to_do:
            initial_energy_list = np.logspace(
                np.log10(args.min_energy),
                np.log10(args.max_energy),
                args.num_energy_pts,
            )
        ...
        for mV in args.mV:
            ...
            initial_energy_list = np.logspace(
                np.log10(min_energy), np.log10(args.max_energy), args.num_energy_pts
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Produce VEGAS integrators for various production processes",
    ...
    parser.add_argument(
        ...
        type=str,
        default="unspecified",
        help="target on which to train (Dark Brem. Only)",
    )
    parser.add_argument(
        "-process_targets",
        nargs="+",
        type=str,
        default=["graphite"],
        help="list of targets to process for shower code",
    )

    parser.add_argument(
        "-save_location",
        type=str,
        default="../data/VEGAS_backend/SM/",
    ...
    parser.add_argument(
        ...
        type=str,
        default=["DarkBrem"],
        help='list of processes to be run "all" does whole list, if mV non-zero only DarkBrem \
    ...
    parser.add_argument(
        ...
        type=float,
        default=[0.05],
        help="dark vector mass in GeV (can be a space-separated list)",
    )
    parser.add_argument(
        "-min_energy",
        type=float,
        default=0.01,
        help="minimum initial energy (in GeV) to evaluate (must be larger than mV)",
    )
    parser.add_argument(
        "-max_energy",
        type=float,
        default=100.0,
        help="maximum initial energy (in GeV) to evaluate",
    )
    parser.add_argument(
        "-num_energy_pts",
        type=int,
        default=100,
        help="number of initial energy values to evaluate, scan is done in log space",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="stitch integrators for different energies together",
    )
