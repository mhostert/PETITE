...
#                    'DarkBrem': {'diff_xsection': dsig_dx_dcostheta_dark_brem_exact_tree_level, 'form_factor':Gelastic_inelastic, "QSq_func":darkbrem_qsq},
...
process_info = {
    "PairProd": {"diff_xsection": proc.dsigma_pairprod_dimensionless},
...
def do_find_max_work(params, process_file):
    ...
    integrand = vegas.Integrator(
        map=integrand_or_map, **proc.vegas_integrator_options[event_info["process"]]
    )  # nstrat=nstrat_options[params['process']])
    ...
    samp_dict_info = {
        "neval": params["neval"],
        "max_F": {tm: max_F_TM[tm] for tm in params["process_targets"]},
        "adaptive_map": save_copy,
    }
    ...
    *Later I will create the xsec dictionary and save it in a separate file (sm_xsec.pkl).
    ...
    for process in params["process"]:
        ...
        if process not in ["PairProd", "Comp", "Ann", "Brem", "Moller", "Bhabha"]:
            raise ValueError(
                "Process '",
                process,
                "' not in list of available processes for standard model showers.",
            )
    ...
    for process in params["process"]:
        ...
        for adaptive_map in adaptive_maps:
            sampling, cross_section, incoming_energy = do_find_max_work(
                params, adaptive_map
            )
            ...
            for tm in params["process_targets"]:
                final_xsec_dict[process][tm].append(
                    [incoming_energy, cross_section[tm]]
                )
        # =====------ FIND MAX ------=====####
    ...
    *Later I will create the xsec dictionary and save it in a separate file (sm_xsec.pkl).
    ...
    for mV in params["mV_list"]:
        ...
        for process in params["process"]:
            ...
            if process not in ["DarkBrem", "DarkAnn", "DarkComp"]:
                raise ValueError(
                    "Process '",
                    process,
                    "' not in list of available processes for dark showers.",
                )
        ...
        for process in params["process"]:
            ...
            for adaptive_map in adaptive_maps:
                sampling, cross_section, incoming_energy = do_find_max_work(
                    fm_params, adaptive_map
                )
                # Append sampling dictionary to final sampling dictionary
                final_sampling_dict[mV][process].append(sampling)
                # Append incoming energy and cross section dictionary to final cross section dictionary for each target material
                for tm in params["process_targets"]:
                    final_xsec_dict[mV][process][tm].append(
                        [incoming_energy, cross_section[tm]]
                    )
            # =====------ FIND MAX ------=====####
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Process VEGAS integrators, find maxes etc",
    ...
    parser.add_argument(
        "-import_directory",
        type=str,
        help="directory to import files from (path relative to main PETITE directory)",
        required=True,
    )
    # optional parameters
    ...
    parser.add_argument(
        ...
        action="append",
        default=[6.0],
        help="atomic number of targets to save",
    )
    # parser.add_argument('-Z', type=list, default=[6], help='atomic number of targets to save')
    parser.add_argument(
        "-mT", type=float, default=11.178, help="nuclear target mass in GeV"
    )
    parser.add_argument(
        "-process",
        type=str,
        default="DarkBrem",
        help='processes to be run, if mV non-zero only DarkBrem \
    ...
    parser.add_argument(
        "-save_location",
        type=str,
        default="cooked_integrators",
        help="directory to save integrators in (path relative to main PETITE directory)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="neval value to provide to VEGAS for making integrator objects",
        required=True,
    )
    parser.add_argument(
        "-n_trials",
        type=int,
        default=100,
        help="number of evaluations to perform for estimating cross-section",
        required=True,
    )
    ...
    params = {
        "A": args.A,
        "Z_T": np.unique(args.Z),
        "mT": args.mT,
        "process": processes_to_do,
